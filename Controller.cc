...
void Controller::findRide(){
    ...
    string custId;
    cout << "Enter the ID of the Customer (e.g., C1): ";
    cin >> custId;

    // Check if customer exists
    Customer* customer = rideShare.getCustomer(custId);
    if (!customer) {
    ...
    view.getRideInfo(size, destination);

    // Display the map
    displayMap();

    // Attempt to find a driver
    Driver* driver = rideShare.findRide(customer->getRating(), size, customer->getLocation());
    if (!driver) {
        cout << "No available driver found matching the criteria." << endl;
        return;
    }

    // Update locations and display success message
    driver->setLocation(destination);
    customer->setLocation(destination);

    // Refresh and display the map again
    displayMap();

    // Print driver information and success message
    cout << "Ride confirmed! Driver details:" << endl;
    driver->print();      
}
